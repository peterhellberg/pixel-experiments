...
var (
	maxSpeed     = 5 * globalScale
	desireAmount = 2 * globalScale
	friendRadius = 50 * globalScale
	crowdRadius  = friendRadius / 1.1
	avoidRadius  = 100 * globalScale
	coheseRadius = friendRadius / 0.7
...
func (b *boid) getAvoidDir() pixel.Vec {
	...
	for _, f := range b.friends {
		...
		if d > 0 && d < crowdRadius {
			diff := b.position.Sub(f.position)

			diff = div(diff.Unit(), d)
...
func (b *boid) flock() {
	var (
		...
		noise = pixel.V(rand.Float64()*2-1, rand.Float64()*2-1).Scaled(0.1)
	)
...
func (a *avoid) draw(m *image.RGBA) {
	...
	r := image.Rect(x-3, y-4, x+3, y+4)

	draw.Draw(m, r, &image.Uniform{a.color}, image.ZP, draw.Src)
}

func randomColor() color.RGBA {
	r := uint8(rand.Intn(100)+55) + 50
	g := uint8(rand.Intn(90)+55) + 55
	b := uint8(rand.Intn(80)+55) + 60
...
func dist(a, b pixel.Vec) float64 {
	return math.Sqrt(b.Sub(a).Dot(a))
}
